...
# maximum and minimum rainfall and temperature deviations

max.rain=max(past.clim$rainSD,na.rm=T)
max.temp=max(past.clim$tempSD,na.rm=T)

min.rain=min(past.clim$rainSD,na.rm=T)
min.temp=min(past.clim$tempSD,na.rm=T)


# load projected relative deviations in rainfall and temperatures (see Supporting Material S3.2)

load("stoch.month.rain.Rda")

load("stoch.month.temp.Rda")
...
for(gg in 1:length(groups)){ # first loop: GROUPS
  ...
  for(rc in 1:4){ # second loop: RCP CLIMATE CHANGE SCENARIOS
  
  for(pu in 1:length(par.sub)){ # third loop: SIMULATIONS
    
    years=sample(c(1997:2018),40,replace=T) #randomly sample years
    ...
    #sample a new sequence of years:
    
    seq.year=rep(1:40,each=12)
    ...
    for(i in 2:length(years)){ # first date is alread given
      ...
      ### ADD TEMPERATURE AND RAINFALL DEVIATIONS
      
      data$rainSD=data$rainSD+stoch.month.rain[which(unique(past.clim$year)%in%unique(data$year)),unique(data$month),seq.year[i-1],rc]
      data$tempSD=data$tempSD+stoch.month.temp[which(unique(past.clim$year)%in%unique(data$year)),unique(data$month),seq.year[i-1],rc]
      
      if(unique(data$rainSD)>0) data$rainSD=min(max.rain,unique(data$rainSD))
      
      if(unique(data$rainSD)<0) data$rainSD=max(min.rain,unique(data$rainSD))
      
      if(unique(data$tempSD)>0) data$rainSD=min(max.temp,unique(data$tempSD))
      
      if(unique(data$tempSD)<0) data$rainSD=max(min.temp,unique(data$tempSD))
      ...
      data.immig$rainSD=data.immig$rainSD+stoch.month.rain[which(unique(past.clim$year)%in%unique(data.immig$year)),unique(data.immig$month),seq.year[i],rc]
      data.immig$tempSD=data.immig$tempSD+stoch.month.temp[which(unique(past.clim$year)%in%unique(data.immig$year)),unique(data.immig$month),seq.year[i],rc]
      
      if(unique(data.immig$rainSD)>0) data.immig$rainSD=min(max.rain,unique(data.immig$rainSD))
      
      if(unique(data.immig$rainSD)<0) data.immig$rainSD=max(min.rain,unique(data.immig$rainSD))
      
      if(unique(data.immig$tempSD)>0) data.immig$rainSD=min(max.temp,unique(data.immig$tempSD))
      
      if(unique(data.immig$tempSD)<0) data.immig$rainSD=max(min.temp,unique(data.immig$tempSD))
